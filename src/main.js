...
export async function scrape(brand) {
  ...
  try {
    ...
    try {
      ...
      console.log(`🔍 Products info not found, starting search for ${brand}...`);
      const entries = await readTxtLinesToArray(`/home/bchk/dev/scrape/src/entries/${brand}.txt`);
    ...
    console.log(`✅ Completed scraping for ${brand}. Found ${productsData.length} products.`);
    return productsData;
  ...
  for (let i = 0; i < toScrape.length; i++) {
    ...
    try {
      ...
      if (i > 0) {
        ...
        console.log(`⏳ Waiting ${Math.round(brandDelay / 1000)}s before next brand...`);
